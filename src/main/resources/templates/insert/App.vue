              ...
              <div class="ibox-content">
                <form class="form-horizontal" name="frm">
                  <div class="form-group"><label class="col-sm-2 control-label">작성자</label>
                    <div class="col-sm-3"><input type="text" class="form-control" ref="writer" v-model="sendData.writer"
                                                 placeholder="작성자를 입력해주세요."></div>
                  </div>
                  <div class="form-group"><label class="col-sm-2 control-label">제목</label>
                    <div class="col-sm-3"><input type="text" class="form-control" ref="title" v-model="sendData.title"
                                                 placeholder="제목을 입력해주세요.">
                    </div>
                  </div>
                  <div class="form-group"><label class="col-sm-2 control-label">비밀번호</label>
                    <div class="col-sm-3"><input type="password" class="form-control" ref="password"
                                                 v-model="sendData.password" placeholder="비밀번호를 입력해주세요."></div>
                  </div>
                  <div class="form-group"><label class="col-sm-2 control-label">상세내용</label>
                    <div class="col-sm-6"><textarea class="form-control" rows="10" ref="content"
                                                    v-model="sendData.content" placeholder="상세 내용을 입력해주세요."></textarea>
                    </div>
                  </div>
                  ...
                  <div class="form-group">
                    <div class="col-sm-4 col-sm-offset-2" style="float: right;">
                      ...
                      <button class="btn btn-success" type="submit" ref="insertBtn" v-on:click="insertPost($event)" v-if="!flags">작성
                      </button>
                      <button class="btn btn-success" type="submit" v-on:click="modifyPost($event)" v-if="flags">수정
                      </button>
                    </div>
                  </div>
                </form>
  ...
  export default {
    ...
    methods: {
      ...
      insertPost: function (event) {
        this.$refs.insertBtn.disabled = true;
        if (!this.validation(event)) {
          this.$refs.insertBtn.disabled = false;
          return;
        }
        // 서버로 FormData 전송.
        // 폼에 입력한 data binding
      ...
      modifyPost(event) {
        ...
        if (this.myCroppa.getChosenFile()) {
          ...
          // croppa validation
          let reg = /(.*?)\.(jpg|JPG|JPEG|jpeg|png|PNG|gif|GIF|bmp|BMP)$/;
          // 정규식
          if (!reg.test(this.sendData.insertParam.fileName)) {
      ...
      onFileTypeMismatch(file){
        console.log(file);
        alert('이미지 파일만 업로드 가능합니다. 이미지 파일을 업로드 해주세요.');
      }
    },
    created() {
